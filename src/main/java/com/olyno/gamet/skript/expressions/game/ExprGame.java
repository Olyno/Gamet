...
import com.olyno.gamet.skript.effects.sections.SecStartGame;
import com.olyno.gamet.skript.effects.sections.SecStopGame;
...
public class ExprGame extends SimpleExpression<Game> {
    ...
    protected Game[] get( Event e ) {
		...
		if (!currentGame.replaceAll(" ", "").isEmpty()) {
			...
			Gamet.error("A game can't have a empty name (Current name: \"" + currentGame + "\")");
            return null;
        }
    }
		...
		if (Game.getGames().containsKey(currentGame)) {
            ...
            for (Object obj : delta) {
                if (obj instanceof Player) {
                    ...
                    switch (mode) {
                        ...
                        case ADD:
							if (game.getMaxPlayer() > game.getPlayers().size()) {
								...
								Gamet.error("The game " + game.getName() + " can't add more players, you have already " + game.getPlayers().size() + " players in this game and the maximum of players is " + game.getMaxPlayer());
                            }
                            break;
                        case REMOVE:
                    ...
                    Gamet.error("Only players can be added in a game, but you used a " + obj.getClass());
                }
            }
        }
    }
