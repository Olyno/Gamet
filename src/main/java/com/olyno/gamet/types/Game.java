...
import com.olyno.gamet.util.game.GameManager;
import com.olyno.gamet.util.game.GameRunner;
...
public class Game extends GameManager {
	...
	private List<Game> sessions = new LinkedList<>();
	...
	private HashMap<Integer, GameSound> timerSounds = new HashMap<>();

	private String name;
	private String displayName;
	private Integer sessionsAmount;
	...
	public Game(String name) {
		...
		this.timerSounds.put(20, new GameSound("entity.experience_orb.pickup"));
		this.timerSounds.put(15, new GameSound("entity.experience_orb.pickup"));
		for (int index = 1; index < 11; index++) {
			this.timerSounds.put(index, new GameSound("entity.experience_orb.pickup"));
		}
		this.isSession = false;
		this.setSessionsAmount(1);
	...
	public Game delete() {
		...
		File gameFile = new File(Bukkit.getServer().getPluginManager().getPlugin("Gamet").getDataFolder(), "Games/" + this.name + ".yml");
	 ...
	 * Sessions amount
	 *
	 * @return The amount of session of the game.
	 */
	public Integer getSessionsAmount() {
	 ...
	 * Set sessions amount
	 *
	 * @param sessionsAmount The amount of session to create for the game.
	 */
	public void setSessionsAmount(Integer sessionsAmount) {
	 ...
	 * The session
	 *
	 * @return The selected session of the game
	 */
	public Game getSession(Integer session) {
		try {
			return sessions.get(session - 1);
		} catch (IndexOutOfBoundsException ex) {
			Gamet.error("Session " + session + " of game \"" + this.getName() + "\" doesn't exist.");
			return null;
		}
	}
	 ...
	 * Is Session
	 *
	 * @return A boolean if the game is a session or not
	 */
	...
	public Team getWinner() {
		if (this.winner == null) {
			for (Team team : this.getTeams().values()) {
				if (this.winner == null) {
					this.winner = team;
				} else if (this.winner.getPoints().getPoints() < team.getPoints().getPoints()) {
					this.winner = team;
				}
			}
		}
	 ...
	 * The Timer Sounds
	 *
	 * @return A hashmap of all sounds during the timer
	 */
	public HashMap<Integer, GameSound> getTimerSounds() {
		return timerSounds;
	}
