...
public class Signs implements Listener {
	...
	public void onGameSignPlaced(SignChangeEvent event) {
		if (Gamet.manage_automatically) {
			if (event.getLine(0).equalsIgnoreCase("[Gamet]")) {
				String game = event.getLine(2);
				if (!Game.getGames().containsKey(game)) {
					event.getPlayer().sendMessage(ChatColor.RED + "[Gamet] The game " + ChatColor.BLUE + game + ChatColor.RED + " doesn't exist. Remember about create it.");
				}
